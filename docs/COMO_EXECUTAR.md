...
Foram adicionados os scripts `scripts\iniciar_erp.bat` e `scripts\abrir_erp.vbs`.

### Como usar
1. Dê duplo clique em `scripts\abrir_erp.vbs` para abrir o ERP **sem deixar janela de CMD aberta**.
2. Na primeira vez, o inicializador prepara o ambiente automaticamente.
3. Nas próximas execuções, apenas atualiza banco (migrations) e abre o ERP.

> Se quiser ver logs de erro no terminal, execute `scripts\iniciar_erp.bat`.
...
1. Clique com botão direito em `scripts\abrir_erp.vbs`.
